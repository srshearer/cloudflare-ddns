...
log.addHandler(ch)
MAX_LOG_SIZE_BYTES = 5000000  # 5,000,000 bytes == 5 MB
...
def get_zone_records(zone_uuid):
    ...
    while current_page != total_pages:
        ...
        r = requests.get(
            f"{API_ENDPOINT}zones/{zone_uuid}/dns_records",
            headers=API_HEADERS,
            params=payload,
        )
...
def update_record(zone_record, local_record, resolving_method):
    ...
    r = requests.put(
        f"{API_ENDPOINT}zones/{zone_record.get('zone_id')}/dns_records/{zone_record.get('id')}",
        headers=API_HEADERS,
        json=payload,
    )
    ...
    if not success:
        log.critical(f"An error occured whilst trying to update '{name}' ({record_type}) record")
        return
...
def main():
    ...
    for zone in set(args.zone):
        config_path = Path.joinpath(CURRENT_DIR, "zones", f"{zone}.yml")

        if not config_path.exists():
            log.critical(f"Zone '{zone}' not found")
            return
        ...
        global API_HEADERS
        API_HEADERS = {"X-Auth-Key": cf_api_key, "X-Auth-Email": cf_email}

        # Get zone informations
        payload = {"name": cf_zone}
        r = requests.get(f"{API_ENDPOINT}zones", headers=API_HEADERS, params=payload)
        data = r.json().get("result")
        if not data:
        ...
        # Logging
        log_file_path = Path.joinpath(CURRENT_DIR, "logs", f"{cf_zone_name}.log")
        fh = RotatingFileHandler(log_file_path, maxBytes=MAX_LOG_SIZE_BYTES, backupCount=10)
        ...
        for records in cf_records:
            for record_name in records:
                ...
                if record_name == "@":
                    ...
                    name = f"{record_name}.{cf_zone_name}"
